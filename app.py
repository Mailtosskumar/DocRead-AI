...
<style>
    ...
    .hero h1 { font-size: 2.4rem; font-weight: 700; margin: 0 0 0.4rem 0; letter-spacing: -0.5px; }
    ...
    div[data-testid="stSidebarContent"] .stMarkdown a { color: #93c5fd !important; }
...
Answer:"""
    ...
    for n, step in enumerate([
        ...
        "Query matched via semantic search",
        "Llama 3 generates grounded answer"
    ], 1):
        st.markdown(f'<div style="display:flex;align-items:flex-start;margin-bottom:6px"><span class="step-num">{n}</span><span style="font-size:0.82rem;line-height:1.4">{step}</span></div>', unsafe_allow_html=True)
        ...
        if item.get("sources"):
            with st.expander("📎 View Sources"):
                for i, src in enumerate(item["sources"], 1):
                    ...
                    st.markdown(f'<span class="source-chip">Page {pg + 1}</span>', unsafe_allow_html=True)
...
if st.session_state.processed:
    ...
    <div class="card" style="text-align:center;padding:3rem;">
        ...
        <div style="margin-top:1.2rem">
            <a href="https://console.groq.com" target="_blank" style="background:#2563eb;color:white;padding:0.5rem 1.2rem;border-radius:8px;text-decoration:none;font-weight:600;font-size:0.9rem">
